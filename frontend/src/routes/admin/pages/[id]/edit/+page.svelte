...
<script lang="ts">
	...
	import { ArrowLeftIcon, Eye, EyeOff, Trash2Icon } from "lucide-svelte";
...
<div class="min-h-screen bg-background py-8 px-4">
	<div class="grid {showPreview ? 'grid-cols-2' : 'grid-cols-1'} gap-6 max-w-[1800px] mx-auto transition-all duration-300">
		<!-- Editor Panel -->
		<form onsubmit={handleSubmit} class="w-full">
			<div class="bg-[--color-surface] rounded-lg shadow-lg">
				<div class="border-b border-gray-700 px-8 py-6">
					<div class="flex items-center justify-between">
						<div>
							<button
								type="button"
								class="text-[--color-text-secondary] hover:text-[--color-text-primary] mb-4 flex items-center gap-2 transition-colors"
							...
							<p class="text-[--color-text-secondary] mt-2">
						...
						<div class="flex items-center gap-3">
							<button
								type="button"
								class="px-4 py-2 {showPreview ? 'bg-blue-600 text-white' : 'text-blue-600 hover:bg-blue-50'} rounded-lg font-medium transition-colors flex items-center gap-2"
						...
						<div class="space-y-6">
							<div class="grid grid-cols-2 gap-6">
								<div>
									<label
										for="title"
										class="block text-sm font-medium text-[--color-text-primary] mb-2"
									>
							...
							<div class="border-t border-[--color-border] pt-6">
								<h2 class="text-xl font-semibold text-[--color-text-primary] mb-4">
									Content Blocks
								</h2>
							...
							<div class="flex gap-6">
								<label class="flex items-center gap-2">
									<input
										...
										class="w-4 h-4 text-blue-600 border-[--color-border] rounded focus:ring-blue-500"
									/>
			...
			<div class="flex justify-end gap-4 mt-6">
				<button
					type="button"
					class="px-6 py-2 border border-[--color-border] rounded-lg text-[--color-text-primary] hover:bg-[--color-surface] font-medium transition-colors"
					onclick={() => goto("/admin/pages")}
				>
					Cancel
				</button>
		...
		<!-- Preview Panel -->
		{#if showPreview}
			<div class="sticky top-8 h-[calc(100vh-4rem)] overflow-hidden">
				<div class="bg-surface rounded-lg shadow-lg h-full flex flex-col">
					<div class="border-b border-input-border px-6 py-4">
						<h2 class="text-xl font-semibold text-[--color-text-primary]">Live Preview</h2>
						<p class="text-sm text-[--color-text-secondary] mt-1">
							Changes update automatically
						</p>
					</div>
					<div class="flex-1 overflow-y-auto p-6 bg-[--color-background]">
