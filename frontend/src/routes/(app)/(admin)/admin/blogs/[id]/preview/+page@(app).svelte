<!-- frontend/src/routes/admin/blogs/[id]/preview/+page.svelte -->
<script lang="ts">
...
<svelte:head>
	<title>Preview: {data.blog?.title}</title>
</svelte:head>
...
<div class="sticky top-0 z-50 border-b border-yellow-400 bg-yellow-50 px-4 py-3 shadow-sm">
	<div class="mx-auto flex max-w-7xl items-center justify-between">
		...
		<div class="flex items-center gap-2">
			<button
				onclick={() => goto(`/admin/blogs/${data.blog.id}/edit`)}
...
<div class="mx-auto max-w-7xl px-4 py-8">
	<article>
		<!-- Page Header -->
		<header class="mb-12">
			<h1 class="mb-4 text-4xl font-bold">{data.blog.title}</h1>
			{#if data.blog.seo?.meta_description}
				<p class="text-xl text-gray-600">{data.blog.seo.meta_description}</p>
			{/if}
		</header>

		<!-- Blocks -->
		{#if data.blog.blocks && data.blog.blocks.length > 0}
			<BlockRenderer blocks={data.blog.blocks} />
		{:else}
